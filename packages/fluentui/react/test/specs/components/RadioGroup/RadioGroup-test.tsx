  ...
  const itemsTest = (getItems: Function, isShorthandApiTest: boolean = true) => {
    ...
    if (isShorthandApiTest) {
      describe('click event handler', () => {
        it('should set "checked" when item is clicked', () => {
          ...
          const wrapper = mountWithProvider(<RadioGroup items={getItems()} checkedValueChanged={checkedValueChanged} />);
          ...
          expect(checkedValueChanged).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ value: 'test-value2' }));
        });
      });
    }
    ...
    it('should not call checkedValueChanged when index did not change', () => {
      ...
      const wrapper = mountWithProvider(
        <RadioGroup items={getItems()} checkedValueChanged={checkedValueChanged} checkedValue="test-value2" />
      );
      const radioGroupItems = wrapper.find('RadioGroupItem');
    ...
    describe('keyDown event handler', () => {
      const testKeyDown = (testName, items, initialValue, keyCode, expectedValue) => {
        it(`keyDown test - ${testName}`, () => {
          const checkedValueChanged = jest.fn();
          const wrapper = mountWithProvider(
            <RadioGroup items={items} checkedValue={initialValue} checkedValueChanged={checkedValueChanged} />
          );
          ...
          expect(checkedValueChanged).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ value: expectedValue }));
        });
      };
